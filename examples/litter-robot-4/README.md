...
- **[`automations.yaml`](automations.yaml)** — ready-to-copy automation examples
  (drawer-full alert, auto clean after the cat leaves, fault alerts, pet-weight
  logging, night-light scheduling).
...
A simple entities card for the device page. The **Start clean cycle** button
drives the motor, so add a tap confirmation to avoid accidental presses.
