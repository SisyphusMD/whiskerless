...
# PYTHON_SHA256 guards the CPython tarball that publish.yml builds the Linux binaries from, and
# Renovate's python-version datasource bumps PYTHON_VERSION without knowing that checksum. Patch
# updates automerge, so without this a Python bump would sail through green and then fail every
# subsequent tagged release inside linux.Dockerfile, where the sha256 is verified.
set -euo pipefail
here="$(cd "$(dirname "$0")" && pwd)"
root="$(cd "$here/.." && pwd)"
publish="$root/.forgejo/workflows/publish.yml"
