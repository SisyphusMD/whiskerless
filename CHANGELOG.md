...
- **Panel sleep mode says what is actually wrong.** The robot only accepts it
  while the weekday sleep schedule is on; enabling it otherwise timed out and
  blamed the wrong setting.
...
- The panel sleep/wake times are documented as read-only on ESP 1.1.75, which
  refuses every write to them.
